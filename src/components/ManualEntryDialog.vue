...
<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
      <h3 class="mb-4 text-lg font-semibold">{{ language.t('addManualSession') }}</h3>
      <div class="space-y-4">
        <div>
          <label class="mb-1 block text-sm font-medium">{{ language.t('typeLabel') }}</label>
          <select v-model="type" class="w-full rounded-lg border px-3 py-2">
            <option value="work">{{ language.t('typeWork') }}</option>
            <option value="break">{{ language.t('typeBreak') }}</option>
          </select>
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium">{{ language.t('dateLabel') }}</label>
          <input type="date" v-model="date" class="w-full rounded-lg border px-3 py-2" />
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="mb-1 block text-sm font-medium">{{ language.t('startLabel') }}</label>
            <input type="time" step="1" v-model="start" class="w-full rounded-lg border px-3 py-2" />
          </div>
          <div>
            <label class="mb-1 block text-sm font-medium">{{ language.t('endLabel') }}</label>
            <input type="time" step="1" v-model="end" class="w-full rounded-lg border px-3 py-2" />
          </div>
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium">{{ language.t('noteLabelOptional') }}</label>
          <input type="text" v-model="note" class="w-full rounded-lg border px-3 py-2" />
        </div>
      </div>
      <div class="mt-6 flex justify-end gap-3">
        <button class="rounded-lg px-4 py-2 text-gray-700 hover:bg-gray-100" @click="open = false">{{ language.t('cancelButton') }}</button>
        <button class="rounded-lg bg-primary-600 px-4 py-2 text-white hover:bg-primary-700" @click="submit">{{ language.t('saveButton') }}</button>
      </div>
    </div>
  </div>
</template>
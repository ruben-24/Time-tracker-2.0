...
const addAdjustment = () => {
  if (!adjDate.value) { alert(language.t('chooseDateAlert')); return }
...
const saveTargetOverride = () => {
  if (!canSaveTargetOverride.value) {
    alert(language.t('chooseCustomTargetDateAlert'))
    return
  }
  timer.setOvertimeTargetOverride(targetOverrideDate.value, targetOverrideTotalMs.value)
  alert(`${language.t('targetFor')} ${targetOverrideDate.value} ${language.t('targetSetTo')} ${formatDuration(targetOverrideTotalMs.value)}.`)
}
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-800 to-slate-900 p-4 safe-top">
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      ...
      <div class="card-glass p-4 text-center">
        <div class="text-sm text-white/70">{{ language.t('globalOffsetLabel') }}</div>
      ...
      <div class="card-glass p-4 text-center">
        <div class="text-sm text-white/70">{{ language.t('totalOvertimeLabel') }}</div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <div class="flex gap-2">
            ...
            <button class="btn btn-primary" @click="saveTarget">{{ language.t('saveButton') }}</button>
          </div>
        </div>
        <div>
          <label class="block text-sm text-white/70 mb-1">{{ language.t('adjustGlobalOffsetLabel') }}</label>
          <div class="flex items-center gap-2">
            <select v-model="offsetSign" class="rounded-lg border border-white/20 bg-white/10 px-2 py-2 text-white">
              <option value="+">+</option>
              <option value="-">-</option>
            </select>
            <input type="number" min="0" v-model.number="offsetHours" class="w-20 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('hoursPlaceholder')" />
            <input type="number" min="0" max="59" v-model.number="offsetMinutes" class="w-24 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('minutesPlaceholder')" />
            <button class="btn btn-amber" @click="saveOffset">{{ language.t('saveButton') }}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid gap-4 mb-6 md:grid-cols-2">
      <div class="card-glass p-4">
        <div class="text-white font-semibold mb-3">{{ language.t('manualAdjustmentTitle') }}</div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          <input type="number" min="0" v-model.number="adjHours" class="w-20 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('hoursPlaceholder')" />
          <input type="number" min="0" max="59" v-model.number="adjMinutes" class="w-24 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('minutesPlaceholder')" />
          <button class="btn btn-emerald" @click="addAdjustment"><Plus class="h-4 w-4" />{{ language.t('addButton') }}</button>
        </div>
      </div>
      ...
      <div class="card-glass p-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <input type="number" min="0" v-model.number="targetOverrideHours" class="w-20 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('hoursPlaceholder')" />
          <input type="number" min="0" max="59" v-model.number="targetOverrideMinutes" class="w-24 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white" :placeholder="language.t('minutesPlaceholder')" />
          <button class="btn btn-primary" :disabled="!canSaveTargetOverride" @click="saveTargetOverride">
            {{ language.t('saveButton') }} ({{ formatDuration(targetOverrideTotalMs) }})
          </button>
        </div>
        <p class="text-xs text-white/60 mt-2">
          {{ language.t('customTargetHelpText') }}
        </p>
      </div>
    </div>
    ...
    <div class="space-y-3">
      <div v-for="d in dailyOvertime" :key="d.date" class="card-glass p-4 flex items-center justify-between gap-4">
        <div>
          <div class="text-white font-semibold">{{ new Date(d.date).toLocaleDateString(language.locale || 'ro-RO') }}</div>
          <div class="text-xs text-white/60">
            {{ language.t('netWork') }}: {{ formatDuration(d.net) }} |
            {{ language.t('targetLabel') }}: {{ formatDuration(d.target) }}
            <span v-if="targetOverrides[d.date]" class="text-emerald-300 ml-1">({{ language.t('customized') }})</span>
          </div>
          <div v-if="d.adjustment" class="text-xs text-white/60">{{ language.t('adjustmentLabel') }}: {{ signed(d.adjustment) }}</div>
        </div>

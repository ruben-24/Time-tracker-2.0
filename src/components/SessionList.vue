      ...
      <li v-for="s in items" :key="s.id" class="rounded-xl border bg-white p-4 shadow-sm">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            >
              {{ s.type === 'work' ? language.t('typeWork') : language.t('typeBreak') }}
            </span>
            <span class="text-sm text-gray-500">{{ s.manual ? language.t('manualLabel') : '' }}</span>
          </div>
          <div class="text-right text-sm text-gray-600">
            <div class="font-medium">{{ formatDuration((s.endedAt ?? Date.now()) - s.startedAt) }}</div>
            <div>{{ formatDateTime(s.startedAt) }} → {{ s.endedAt ? formatDateTime(s.endedAt) : language.t('inProgress') }}</div>
